...
#[bench]
fn world_state_read(b: &mut Bencher) -> Result<()> {
...
#[bench]
fn world_state_write(b: &mut Bencher) -> Result<()> {
...
#[bench]
fn world_block_get(b: &mut Bencher) -> Result<()> {
    let world = BufWorld::new(test_block_system());
...
fn world_sign_read_full(b: &mut Bencher) -> Result<()> {
    ...
    cur.write_sign_lines(&["1234567890123456", "1234567890123456", "1234567890123456", "1234567890123456"])?;
    b.iter(|| {
        let mut output = [String::new(),String::new(),String::new(),String::new()];
        let _ = cur.read_sign_lines(&mut output);
    });
...
fn world_sign_write_full(b: &mut Bencher) -> Result<()> {
    ...
    let mut cur = SignCursor::new(buf);
    b.iter(|| {
        let _ = cur.write_sign_lines(&["1234567890123456", "1234567890123456", "1234567890123456", "1234567890123456"]);
    });
    Ok(())
}
